...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if($id>0){$pdo->prepare('UPDATE poa_hierarchy SET hierarchy_name=?, type=?, threshold_from=?, threshold_to=?, stage_name=?, stage_type=?, stage_sequence=?, approver_role_id=?, parallel_group_id=?, status=?, remarks=?, updated_at=NOW() WHERE id=?')->execute([...$fields,$id]);}
  else {$pdo->prepare('INSERT INTO poa_hierarchy(hierarchy_name,type,threshold_from,threshold_to,stage_name,stage_type,stage_sequence,approver_role_id,parallel_group_id,status,remarks,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())')->execute($fields);} $_SESSION['flash_success']='POA stage rule saved'; header('Location:index.php'); exit;
}
...
$rows=$pdo->query('SELECT p.*, r.role_name FROM poa_hierarchy p LEFT JOIN roles r ON r.id=p.approver_role_id ORDER BY p.hierarchy_name, p.stage_sequence')->fetchAll();
...
<div class="row g-3"><div class="col-md-4"><div class="card p-3"><h6><?= $edit?'Edit':'Add' ?> POA Stage</h6>
...
<div class="col-6"><label class="form-label">Stage Name</label><input class="form-control" name="stage_name" value="<?= e($edit['stage_name']??'') ?>" placeholder="e.g. Signature Approval" required></div>
